...
contract ExchangeState is OwnableOperatorRole {
    ...
    function getCompleted(ExchangeDomain.OrderKey calldata key) external view returns (uint256) {
    ...
    function getCompletedKey(ExchangeDomain.OrderKey memory key) public pure returns (bytes32) {
        return
            keccak256(
                abi.encodePacked(
                    key.owner,
