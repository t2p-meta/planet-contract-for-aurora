...
contract BlindBox is Ownable, BlindDomain {
    ...
    event OpenIndex(uint256 index);

    event Cancel(string indexed key, address owner, address buyToken, uint256 buyTokenId, uint256 salt);
    ...
    function validateTime(BlindBox memory blindbox) internal view {
        require((blindbox.startTime < block.timestamp && block.timestamp < blindbox.endTime), 'incorrect time');
    }

    function validateBlindBoxSig(BlindBox memory blindbox, Sig memory sig) internal pure {
    ...
    function validateBuyerFeeSig(
        ...
        require(
            prepareBuyerFeeMessage(blindbox, buyerFee).recover(sig.v, sig.r, sig.s) == buyerFeeSigner,
    ...
    function verifyState(
        ...
        uint256[] memory completed = state.getCompleted(key);
        if (completed.length == 0) return;
        uint256 total = 0;
        uint256 completedTotal = 0;
        for (uint256 i = 0; i < assetAmounts.length; i++) {
            total = total.add(assetAmounts[i]);
            completedTotal = completedTotal.add(completed[i]);
        }
        uint256 newCompleted = completedTotal.add(amount.mul(opening));
    ...
    function compute(
        ...
        uint256 total = totalAmounts(blindbox);

        uint256 paying = blindbox.buying.mul(amount).div(total.div(blindbox.opening));
        if (blindbox.key.buyAsset.assetType == AssetType.ETH) {
            validateEthTransfer(paying, buyerFee);
        }

        for (uint256 i = 0; i < amount; i++) {
            (uint256[] memory amounts, uint256 stock) = remainAmounts(blindbox);
            require(stock != 0, 'incorrect stock');
            openBox(blindbox, stock, amounts, buyer, buyerFee);
        }

        if (
            blindbox.key.buyAsset.assetType == AssetType.ERC1155 || blindbox.key.buyAsset.assetType == AssetType.ERC721
        ) {
            transferWithFeesPossibility(
                blindbox.key.buyAsset,
                paying,
                msg.sender,
                blackHole,
                true,
    ...
    function totalAmounts(BlindBox memory blindbox) internal pure returns (uint256 total) {
        total = 0;
        for (uint256 i = 0; i < blindbox.assetAmounts.length; i++) {
            require(
                blindbox.key.sellAssets[i].assetType != AssetType.ETH &&
                    blindbox.key.sellAssets[i].assetType != AssetType.ERC20,
                'incorrect sellAsset'
            );
    ...
    function remainAmounts(BlindBox memory blindbox) internal view returns (uint256[] memory amounts, uint256 stock) {
        uint256[] memory completed = state.getCompleted(blindbox.key);
        amounts = new uint256[](blindbox.assetAmounts.length);
        stock = 0;
        for (uint256 i = 0; i < blindbox.assetAmounts.length; i++) {
    ...
    function rewardIndex(uint256[] memory amounts, uint256 stock) internal view returns (uint256 index) {
    ...
    */
    ...
    function openBox(
        ...
        for (uint256 j = 0; j < blindbox.opening; j++) {
            uint256 index = rewardIndex(amounts, stock);
    ...
    function prepareBuyerFeeMessage(BlindBox memory blindbox, uint256 fee) public pure returns (string memory) {
    ...
    function transferWithFeesPossibility(
        ...
        if (!hasFee || (firstType.assetType != AssetType.ETH && firstType.assetType != AssetType.ERC20)) {
