...
async function set_operator() {
  ...
  var filename = './deployed/' + config.chainId + '_' + address + '.json'
  ...
  if (
    ...
    console.log(
      'transferProxy || transferProxyForDeprecated || erc20TransferProxy || exchangeState || exchange contract not deploy'
    )
    process.exit()
  }
  ...
  if (!data.exchange.transferProxyTx) {
    ...
    result = await addOperator(
      data.exchange.address,
  ...
  if (!data.exchange.transferProxyForDeprecatedTx) {
    ...
    if (result.error) {
      console.log(
        'exchange transferProxyForDeprecated operator error',
        result.error
      )
      await utils.writeJsonFile(filename, data)
      process.exit()
    ...
    console.log(
      'add exchange transferProxyForDeprecated operator tx',
      result.tx
    )
  ...
  if (!data.blindBox.transferProxyForDeprecatedTx) {
    ...
    if (result.error) {
      console.log(
        'blindBox transferProxyForDeprecated operator error',
        result.error
      )
      process.exit()
    }
    console.log(
      'add blindBox transferProxyForDeprecated operator tx',
      result.tx
    )
  ...
  if (!data.auctionExchange.transferProxyForDeprecatedTx) {
    ...
    if (result.error) {
      console.log(
        'auctionExchange transferProxyForDeprecated operator error',
        result.error
      )
      process.exit()
    }
    console.log(
      'add auctionExchange transferProxyForDeprecated operator tx',
      result.tx
    )
  ...
  if (!data.auctionExchange.erc20TransferProxyTx) {
    ...
    if (result.error) {
      console.log(
        'auctionExchange erc20TransferProxy operator error',
        result.error
      )
      process.exit()
    }
